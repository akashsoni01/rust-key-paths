...
fn main() {
    ...
    let street_kp = KeyPaths::failable_owned(|p: Person| Some(p.address.street));

    let city_kp = KeyPaths::failable_owned(|p: Person| Some(p.address.city));
    ...
    let failable_addresses_kp =
        KeyPaths::failable_owned(|p: PersonWithAddresses| Some(p.addresses));
