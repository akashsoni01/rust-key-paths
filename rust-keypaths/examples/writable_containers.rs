...
fn main() {
    ...
    println!(
        "  vec_field[1]: {:?}",
        test.vec_field.as_ref().and_then(|v| v.get(1))
    );
    println!("  vecdeque_field[0]: {:?}", test.vecdeque_field.get(0));
    println!(
        "  hashmap_field[\"key1\"]: {:?}",
        test.hashmap_field.get("key1")
    );
    println!(
        "  btreemap_field[\"key2\"]: {:?}",
        test.btreemap_field.get("key2")
    );
    ...
    let chained_vecdeque =
        WritableOptionalKeyPath::new(|c: &mut ContainerTest| Some(&mut c.vecdeque_field))
    ...
    let chained_hashmap =
        WritableOptionalKeyPath::new(|c: &mut ContainerTest| Some(&mut c.hashmap_field))
            .then(hashmap_key_kp);
    ...
    let chained_btreemap =
        WritableOptionalKeyPath::new(|c: &mut ContainerTest| Some(&mut c.btreemap_field))
