...
fn main() {
    ...
    println!("\n=== Keypaths Types ===");
    println!(
        "loading() returns: KeyPath<Status, Status, impl for<\'r> Fn(&\'r Status) -> &\'r Status> (readable)"
    );
    println!(
        "success() returns: KeyPath<Status, String, impl for<\'r> Fn(&\'r Status) -> &\'r String> (failable readable)"
    );
    println!(
        "error() returns: KeyPath<Status, String, impl for<\'r> Fn(&\'r Status) -> &\'r String> (failable readable)"
    );
    println!(
        "data() returns: KeyPath<Status, i32, impl for<\'r> Fn(&\'r Status) -> &\'r i32> (failable readable)"
    );
    println!(
        "position() returns: KeyPath<Status, Status, impl for<\'r> Fn(&\'r Status) -> &\'r Status> (failable readable)"
    );
    println!(
        "user() returns: KeyPath<Status, Status, impl for<\'r> Fn(&\'r Status) -> &\'r Status> (failable readable)"
    );
}
