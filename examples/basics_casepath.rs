...
struct SomeComplexStruct {
    ...
    // Locks inside Arc (SyncKp: root SomeComplexStruct, value SomeOtherStruct)
...
/// Exercise every **sync** `SyncKp` keypath: `get` and `get_mut` on the `SomeOtherStruct` value
/// (exercises `rust_key_paths::sync_kp::SyncKp<…>` paths produced by the derive).
...
// Async `SyncKp` / `Option<Arc<tokio::...>>` combinations: exercise separately with
...
fn main() {
    ...
    // SyncKp for Arc<parking_lot::RwLock<Option<SomeOtherStruct>>> should yield value SomeOtherStruct
