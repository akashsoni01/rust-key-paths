...
impl<T: 'static> FieldConsumer<T> {
        ...
        V: Clone + Send + Sync,
    {
        ...
        self.field_registry
            .insert(name.to_string(), Box::new(accessor));

        if self.debug_mode {
            println!(
                "Registered field '{}' with type {}",
                name,
                std::any::type_name::<V>()
            );
        }
        ...
        V: Send + Sync,
    {
        ...
        if self.debug_mode {
            println!(
                "Registered owned field '{}' with type {}",
                name,
                std::any::type_name::<V>()
            );
        }
    ...
    fn consume_field(&mut self, field_name: &str) -> Option<Box<dyn Any>> {
        ...
        if let Some(accessor) = self.field_registry.get(field_name) {
            if self.debug_mode {
                println!(
                    "Consuming field '{}' of type {}",
                    field_name,
                    accessor.field_type_name()
                );
            }
    ...
    fn debug_info(&self) -> FieldAccessDebugInfo {
        ...
        let available_fields: Vec<String> = self
            .field_registry
            .keys()
    ...
    fn is_field_available(&self, field_name: &str) -> bool {
        self.field_registry.contains_key(field_name) && !self.consumed_fields.contains(field_name)
    }
...
fn main() {
    ...
    println!(
        "Is 'email' available? {}",
        consumer.is_field_available("email")
    );
    ...
    println!(
        ...
        product_consumer.available_fields()
    );
    ...
    println!(
        ...
        order_consumer.available_fields()
    );
    ...
    println!(
        "Is 'nonexistent' available? {}",
        advanced_consumer.is_field_available("nonexistent")
    );
    ...
    println!(
        ...
        test_consumer.available_fields()
    );
