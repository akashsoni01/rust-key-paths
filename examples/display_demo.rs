...
fn main() {
    ...
    let age_path: OptionalKeyPath<Person, u32, _> = OptionalKeyPath::new(|p: &Person| p.age.as_ref());
    println!("OptionalKeyPath: {}", age_path);
    println!("Debug: {:?}\n", age_path);
    
    // WritableKeyPath (mutable, non-optional)
    let name_writable: WritableKeyPath<Person, String, _> = WritableKeyPath::new(|p: &mut Person| &mut p.name);
    println!("WritableKeyPath: {}", name_writable);
    println!("Debug: {:?}\n", name_writable);
    
    // WritableOptionalKeyPath (mutable, optional)
    let age_writable: WritableOptionalKeyPath<Person, u32, _> = WritableOptionalKeyPath::new(|p: &mut Person| p.age.as_mut());
    ...
    let city_path: OptionalKeyPath<Person, String, _> = OptionalKeyPath::new(|p: &Person| p.address.city.as_ref());
