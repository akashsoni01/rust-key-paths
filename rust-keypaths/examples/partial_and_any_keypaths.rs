...
use rust_keypaths::{
    ...
    PartialKeyPath, PartialOptionalKeyPath,
    AnyKeyPath,
    containers,
};
...
fn main() {
    ...
    println!("  Matches String: {}", partial_name.value_type_id() == TypeId::of::<String>());
    ...
    if let Some(city_any) = partial_hq_city.get(&company) {
        println!("  Headquarters city (via PartialOptionalKeyPath): {:?}", city_any);
    ...
    let any_keypaths: Vec<AnyKeyPath> = vec![
        any_company_name.clone(),
        any_employee_name.clone(),
    ];
    
    println!("  Stored {} keypaths in Vec<AnyKeyPath>", any_keypaths.len());
    println!("  Company name TypeId: {:?}", any_company_name.root_type_id());
    println!("  Employee name TypeId: {:?}", any_employee_name.root_type_id());
    ...
    if let Some(city_any) = partial_complex.get(&company) {
        println!("  Employee department city (via PartialOptionalKeyPath): {:?}", city_any);
    ...
    if let Some(employee) = company.employees.get_mut(0) {
        if let Some(salary_any) = partial_salary.get_mut_as::<f64>(employee) {
            *salary_any = 160000.0;
            println!("  Updated salary via PartialWritableKeyPath: {}", employee.salary);
        }
    }
    
    // Using AnyWritableKeyPath
    let salary_opt_kp = WritableOptionalKeyPath::new(|e: &mut Employee| e.department.as_mut().map(|d| &mut d.budget));
    let any_budget = salary_opt_kp.to_any();
    
    if let Some(employee) = company.employees.get_mut(0) {
        if let Some(Some(budget)) = any_budget.get_mut_as::<Employee, f64>(employee) {
            *budget = 1100000.0;
            println!("  Updated budget via AnyWritableKeyPath: {}", 
    ...
    println!("  Company name keypath - Root: {:?}, Value: {:?}", 
             any1.root_type_id(), any1.value_type_id());
    println!("  Employee name keypath - Root: {:?}, Value: {:?}", 
             any2.root_type_id(), any2.value_type_id());
    ...
    println!("   - Ensuring correct usage (e.g., PartialKeyPath<User> can only be used with &User)");
