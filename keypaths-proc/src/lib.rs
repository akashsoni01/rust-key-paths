    ...
    let methods = match input.data {
        Data::Struct(data_struct) => match data_struct.fields {
            Fields::Named(fields_named) => {
                ...
                for field in fields_named.named.iter() {
                    ...
                    match (kind, inner_ty.clone()) {
                        (WrapperKind::Option, Some(inner_ty)) => {
                            // For Option<T>, return FailableCombinedKeyPath with all three access patterns
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.as_ref(),
                                        |s: &mut #name| s.#field_ident.as_mut(),
                                        |s: #name| s.#field_ident,
                                    )
                                }
                            });
                        }
                        (WrapperKind::Vec, Some(inner_ty)) => {
                            // For Vec<T>, return FailableCombinedKeyPath to first element
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.first(),
                                        |s: &mut #name| s.#field_ident.first_mut(),
                                        |s: #name| s.#field_ident.into_iter().next(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::HashMap, Some(inner_ty)) => {
                            // For HashMap<K,V>, return FailableCombinedKeyPath to the container
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                                        |s: &mut #name| Some(&mut s.#field_ident),
                                        |s: #name| Some(s.#field_ident),
                                    )
                                }
                            });
                        ...
                        (WrapperKind::BTreeMap, Some(inner_ty)) => {
                            // For BTreeMap<K,V>, return FailableCombinedKeyPath to the container
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                        ...
                        (WrapperKind::Box, Some(inner_ty)) => {
                            // For Box<T>, return FailableCombinedKeyPath to inner type
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&*s.#field_ident),
                        ...
                        (WrapperKind::Rc, Some(inner_ty)) | (WrapperKind::Arc, Some(inner_ty)) => {
                            // For Rc<T>/Arc<T>, return FailableCombinedKeyPath to inner type (readable only, no mutable/owned)
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&*s.#field_ident),
                                        |_s: &mut #name| None, // Rc/Arc don't support mutable access
                                        |_s: #name| None, // Rc/Arc don't support owned access without cloning
                                    )
                                }
                            });
                        }
                        (WrapperKind::HashSet, Some(inner_ty)) => {
                            // For HashSet<T>, return FailableCombinedKeyPath to any element
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.iter().next(),
                                        |_s: &mut #name| None, // HashSet doesn't support direct mutable access
                                        |s: #name| s.#field_ident.into_iter().next(),
                                    )
                                }
                            });
                        ...
                        (WrapperKind::BTreeSet, Some(inner_ty)) => {
                            // For BTreeSet<T>, return FailableCombinedKeyPath to any element
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.iter().next(),
                                        |_s: &mut #name| None, // BTreeSet doesn't support direct mutable access
                                        |s: #name| s.#field_ident.into_iter().next(),
                                    )
                                }
                            });
                        ...
                        (WrapperKind::VecDeque, Some(inner_ty)) => {
                            // For VecDeque<T>, return FailableCombinedKeyPath to front element
                            // Note: front_mut() returns a guard that can't outlive the closure, so mutable access returns None
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.front(),
                                        |_s: &mut #name| None, // VecDeque::front_mut() returns a guard that can't outlive the closure
                                        |s: #name| s.#field_ident.pop_front(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::LinkedList, Some(inner_ty)) => {
                            // For LinkedList<T>, return FailableCombinedKeyPath to front element
                            // Note: LinkedList doesn't have front_mut(), so mutable access returns None
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.front(),
                                        |_s: &mut #name| None, // LinkedList doesn't support direct mutable access to front
                                        |s: #name| s.#field_ident.pop_front(),
                                    )
                                }
                            });
                        ...
                        (WrapperKind::BinaryHeap, Some(inner_ty)) => {
                            // For BinaryHeap<T>, return FailableCombinedKeyPath to peek element
                            // Note: peek_mut() returns a guard that can't outlive the closure, so mutable access returns None
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.peek(),
                                        |_s: &mut #name| None, // BinaryHeap::peek_mut() returns a guard that can't outlive the closure
                                        |s: #name| s.#field_ident.pop(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::Result, Some(inner_ty)) => {
                            // For Result<T, E>, return FailableCombinedKeyPath to Ok value
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#field_ident.as_ref().ok(),
                                        |s: &mut #name| s.#field_ident.as_mut().ok(),
                                        |s: #name| s.#field_ident.ok(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::Mutex, Some(inner_ty)) => {
                            // For Mutex<T>, return FailableCombinedKeyPath to the container (not inner type due to lifetime issues)
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                        ...
                        (WrapperKind::RwLock, Some(inner_ty)) => {
                            // For RwLock<T>, return FailableCombinedKeyPath to the container (not inner type due to lifetime issues)
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                        ...
                        (WrapperKind::Weak, Some(inner_ty)) => {
                            // For Weak<T>, return FailableCombinedKeyPath to the container (not inner type due to lifetime issues)
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                        ...
                        (WrapperKind::None, None) => {
                            // For basic types, return FailableCombinedKeyPath
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
                        ...
                        _ => {
                            // For unknown types, return FailableCombinedKeyPath
                            tokens.extend(quote! {
                                pub fn #field_ident() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#field_ident),
            ...
            Fields::Unnamed(unnamed) => {
                ...
                for (idx, field) in unnamed.unnamed.iter().enumerate() {
                    ...
                    match (kind, inner_ty.clone()) {
                        (WrapperKind::Option, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#idx_lit.as_ref(),
                                        |s: &mut #name| s.#idx_lit.as_mut(),
                                        |s: #name| s.#idx_lit,
                                    )
                                }
                            });
                        ...
                        (WrapperKind::Vec, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#idx_lit.first(),
                                        |s: &mut #name| s.#idx_lit.first_mut(),
                                        |s: #name| s.#idx_lit.into_iter().next(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::HashMap, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #ty, impl for<'r> Fn(&'r #name) -> Option<&'r #ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #ty> + 'static, impl Fn(#name) -> Option<#ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| Some(&s.#idx_lit),
                                        |s: &mut #name| Some(&mut s.#idx_lit),
                                        |s: #name| Some(s.#idx_lit),
                                    )
                                }
                            });
                        ...
                        (WrapperKind::VecDeque, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#idx_lit.front(),
                                        |_s: &mut #name| None, // VecDeque::front_mut() returns a guard that can't outlive the closure
                                        |s: #name| s.#idx_lit.pop_front(),
                                    )
                                }
                            });
                        }
                        ...
                        (WrapperKind::BinaryHeap, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#idx_lit.peek(),
                                        |_s: &mut #name| None, // BinaryHeap::peek_mut() returns a guard that can't outlive the closure
                                        |s: #name| s.#idx_lit.pop(),
                                    )
                                }
                            });
                        }
                        (WrapperKind::Result, Some(inner_ty)) => {
                            tokens.extend(quote! {
                                pub fn #field_name() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| s.#idx_lit.as_ref().ok(),
                                        |s: &mut #name| s.#idx_lit.as_mut().ok(),
                                        |s: #name| s.#idx_lit.ok(),
                                    )
                                }
                            });
        ...
        Data::Enum(data_enum) => {
            ...
            for variant in data_enum.variants.iter() {
                ...
                match &variant.fields {
                    Fields::Unit => {
                        // Unit variant - return FailableCombinedKeyPath that matches this variant
                        tokens.extend(quote! {
                            pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #name, impl for<'r> Fn(&'r #name) -> Option<&'r #name> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #name> + 'static, impl Fn(#name) -> Option<#name> + 'static> {
                                rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                    |s: &#name| match s {
                                        #name::#v_ident => Some(s),
                                        _ => None,
                    ...
                    match (kind, inner_ty.clone()) {
                                (WrapperKind::Option, Some(inner_ty)) => {
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                |s: &#name| match s {
                                    ...
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                ...
                                                |s: #name| match s {
                                                    #name::#v_ident(inner) => inner.into_iter().next(),
                                                    _ => None,
                                                },
                                            )
                                        }
                                ...
                                (WrapperKind::HashMap, Some(inner_ty)) => {
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #field_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #field_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #field_ty> + 'static, impl Fn(#name) -> Option<#field_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                |s: &#name| match s {
                                    ...
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                ...
                                                |_s: &mut #name| None, // VecDeque::front_mut() returns a guard that can't outlive the closure
                                                |s: #name| match s {
                                                    #name::#v_ident(inner) => inner.pop_front(),
                                                    _ => None,
                                                },
                                            )
                                        }
                                    ...
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #inner_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #inner_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #inner_ty> + 'static, impl Fn(#name) -> Option<#inner_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                ...
                                                |s: &mut #name| match s {
                                                    #name::#v_ident(inner) => inner.as_mut().ok(),
                                                    _ => None,
                                                },
                                                |s: #name| match s {
                                                    #name::#v_ident(inner) => inner.ok(),
                                                    _ => None,
                                                },
                                            )
                                        }
                                ...
                                (WrapperKind::None, None) => {
                                    // Basic type - return FailableCombinedKeyPath
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #field_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #field_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #field_ty> + 'static, impl Fn(#name) -> Option<#field_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                |s: &#name| match s {
                                                    #name::#v_ident(ref inner) => Some(inner),
                                                    _ => None,
                                                },
                                                |s: &mut #name| match s {
                                                    #name::#v_ident(ref mut inner) => Some(inner),
                                                    _ => None,
                                                },
                                                |s: #name| match s {
                                                    #name::#v_ident(inner) => Some(inner),
                                ...
                                _ => {
                                    // Unknown type - return FailableCombinedKeyPath
                                    tokens.extend(quote! {
                                        pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #field_ty, impl for<'r> Fn(&'r #name) -> Option<&'r #field_ty> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #field_ty> + 'static, impl Fn(#name) -> Option<#field_ty> + 'static> {
                                            rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                                |s: &#name| match s {
                            ...
                            // Multi-field tuple variant - return FailableCombinedKeyPath to the variant
                            tokens.extend(quote! {
                                pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #name, impl for<'r> Fn(&'r #name) -> Option<&'r #name> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #name> + 'static, impl Fn(#name) -> Option<#name> + 'static> {
                                    rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                        |s: &#name| match s {
                    ...
                    Fields::Named(_named) => {
                        // Named field variant - return FailableCombinedKeyPath to the variant
                        tokens.extend(quote! {
                            pub fn #snake() -> rust_keypaths::FailableCombinedKeyPath<#name, #name, impl for<'r> Fn(&'r #name) -> Option<&'r #name> + 'static, impl for<'r> Fn(&'r mut #name) -> Option<&'r mut #name> + 'static, impl Fn(#name) -> Option<#name> + 'static> {
                                rust_keypaths::FailableCombinedKeyPath::failable_combined(
                                    |s: &#name| match s {
