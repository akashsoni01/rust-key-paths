...
fn main() {
    ...
    let user_name_keypath = Profile::user_r()
        .to_optional()
    ...
    fn parking_mutex_adapter<F>(
        keypath: KeyPath<User, String, impl for<'r> Fn(&'r User) -> &'r String>,
        mutex: &Mutex<User>,
        f: F,
    ) where
        F: FnOnce(&String),
    ...
    fn parking_rwlock_adapter<F>(
        keypath: KeyPath<Profile, String, impl for<'r> Fn(&'r Profile) -> &'r String>,
        rwlock: &RwLock<Profile>,
        f: F,
    ) where
        F: FnOnce(&String),
    {
    ...
    {
        let name_keypath = User::name_r();
        let name = with_parking_mutex(name_keypath, &parking_mutex_user, |name: &String| {
            name.clone()
        });
    ...
    {
        let bio_keypath = Profile::bio_r();
        let bio = with_parking_rwlock(bio_keypath, &parking_rwlock_profile, |bio: &String| {
            bio.clone()
        });
    ...
    let nested_name_keypath = Profile::user_r()
        .to_optional()
        .then(User::name_r().to_optional());
    {
        let guard = parking_rwlock_profile.read();
